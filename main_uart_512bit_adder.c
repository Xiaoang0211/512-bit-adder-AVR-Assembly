...
int main (void)
{
   ...
   uint8_t c[64];
    ...
    //initialization of the number c with 0
   ...
   while(1)
   {
       uart_writeStrBlocking("Hello World! \n");
       uart_readBlocking((uint8_t*)&a,64);
       uart_writeStrBlocking("a=");
       uart_writeBlocking((uint8_t*)&a,64);

       uart_readBlocking((uint8_t*)&b,64);
       uart_writeStrBlocking("b=");
       uart_writeBlocking((uint8_t*)&b,64);
       add512(c,a,b);
       uart_writeStrBlocking("c=");      
